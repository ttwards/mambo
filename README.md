...
Mambo 使用 West 管理 Zephyr 和外部模块。`west.yml` 将 Zephyr 固定到已经通过样例构建矩阵验证的提交：

```text
6085aadeb337f27ee7411fa2ecbe8cf49164e360
```

固定提交可以避免 Zephyr `main` 分支变化导致构建结果随时间漂移。升级 Zephyr 时应在独立分支中修改 `west.yml`，执行 `west update`，并至少验证以下构建组合：

```shell
west build -p always -b robomaster_board_c samples/motor/dji_m3508_demo
west build -p always -b robomaster_board_a samples/motor/dji_m3508_demo
west build -p always -b robomaster_board_c samples/motor/dm_demo
west build -p always -b robomaster_board_a samples/motor/dm_demo
west build -p always -b dm_mc02 samples/motor/dm_demo
```
